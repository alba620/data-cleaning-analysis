...
Se trata de `r sum(is.na(selected.houses$rooms))` registros que no tienen valor en el campo *rooms*. Al ser tantos registros (`r sum(is.na(selected.houses$rooms))*100.0/selected.houses.dim[1]`% del total de registros) se decide no utilizar el campo. Otras opciones incluyen eliminar el campo completamente del *dataset*, predecir el número de habitaciones dado el resto de características o imputarles un valor arbitrario, como 0.
...
```
El campo *source* contiene solo los valores esperados. El campo *location* contiene un nivel incorrecto y el campo *floor* contiene información redundante en varios niveles, además de un nivel incorrecto. Se decide no utilizar la información de si la vivienda es interior o exterior y si posee o no ascensor; por tanto, se cambiará el tipo de dato de *floor* a entero.
...
Al eliminar los niveles no utilizados en los factores, se han eliminado los niveles incorrectos de *location* y *floor*. Ahora solo hace falta modificar el campo *floor* para obtener únicamente la planta de la vivienda. Si la vivienda no tiene planta (el caso de "Sin planta"), se le asigna la planta 0.

```{r}
selected.houses <- selected.houses %>% dplyr::mutate(floor=as.character(floor)) %>%
  dplyr::mutate(floor=ifelse(floor=='Sin planta', '0', floor)) %>%
  dplyr::mutate(floor=readr::parse_number(floor))
unique(selected.houses$floor)
```
