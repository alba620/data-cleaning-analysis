...
El conjunto de datos objeto de análisis de este trabajo es ["*Houses for sale in the Salamanca and Villaverde district of Madrid in April 2022*"](https://zenodo.org/record/6423459), un *dataset* de las viviendas en venta en los portales inmobiliarios de Idealista y Fotocasa en el distrito más caro (Salamanca) y el distrito más barato (Villaverde) de Madrid, obtenidos en la Práctica 1 de esta asignatura.

A partir de los datos inmobiliarios se pretende encontrar relaciones entre las características físicas de las viviendas; principalmente se investigan las relaciones que puedan tener las otras características con el precio de la vivienda. Además de describir las relaciones entre el precio y las viviendas, se busca realizar una comparativa de la situación del mercado inmobiliario entre ambos distritos.

Concretamente, se va a responder a las siguientes preguntas:
...
- ¿Existen diferencias significativas en el precio de la vivienda según el barrio en el que esté localizada? Para responder esta pregunta se realizará un **contraste de hipótesis** con un nivel de significancia $\alpha$ = 0.05.
...
La información que se desprenda de este estudio es relevante para **agentes del mercado inmobiliario** -- agencias inmobiliarias, propietarios particulares que buscan vender un inmueble y personas que buscan comprar una vivienda, -- así como para terceros que estén interesados en la situación del mercado inmobiliario en dichas zonas de Madrid. Por último, este estudio también es relevante dentro de los **temas sociales**, como repartición de la riqueza y posibilidad de acceso a la vivienda.
...
cor(corr.houses, use='complete.obs', method='pearson')

cor.test(corr.houses$price, corr.houses$m2, method='spearman')
cor.test(corr.houses$price, corr.houses$m2, method='pearson')
cor.test(corr.houses$price, corr.houses$desc.len, method='spearman')
cor.test(corr.houses$price, corr.houses$desc.len, method='pearson')
cor.test(corr.houses$price, corr.houses$price.m2, method='spearman')
cor.test(corr.houses$price, corr.houses$price.m2, method='pearson')
cor.test(corr.houses$price.m2, corr.houses$m2, method='pearson')
...
ggcorr(corr.houses, method=c('complete.obs', 'pearson'), name='Corr. coef.',
       label=TRUE, label_alpha=TRUE, legend.position='left') +
...
### ¿Existen diferencias en el precio de la vivienda según el barrio?
...
El histograma, el gráfico de densidad y el *boxplot* muestran que el precio por metro cuadrado en el barrio de Salamanca es superior al precio por metro cuadrado del distrito de Villaverde.
...
Por tanto, la **pregunta de investigación** es si existe una diferencia significativa en el **precio por metro cuadrado** entre las viviendas altas y bajas. Como ya se ha determinado anteriormente, el precio por m$^2$ entre barrios es significativamente diferente, por lo que se prefiere realizar la investigación en cada barrio de forma separada. En consecuencia, se plantean las siguientes **hipótesis nula y alternativa**:  
...
El histograma, el gráfico de densidad y el *boxplot* muestran que el precio por metro cuadrado en el barrio de Salamanca es aproximadamente el mismo para los inmuebles situados en plantas altas y bajas.
...
El histograma, el gráfico de densidad y el *boxplot* muestran que el precio por metro cuadrado en el barrio de Villaverde es aproximadamente el mismo para los inmuebles situados en plantas altas y bajas.
...
t.test(upper.floors.villaverde.houses$price.m2,
       ...
       alternative = "greater")
```
El valor de **`p-value`** es 0.06, por encima de $\alpha$ = 0.05, por lo que se **acepta la hipótesis nula** y se rechaza la hipótesis alternativa: el precio por m$^2$ en el barrio de Villaverde es similar para viviendas en últimas plantas y en viviendas bajas.
...
En **conclusión**, la altura de la vivienda tiene influencia en el precio por m$^2$ en el barrio de Salamanca, pero no en el de Villaverde. Por tanto, el precio por metro cuadrado de las viviendas altas en el barrio de Salamanca es superior al precio por metro cuadrado de las viviendas bajas en el mismo barrio; sin embargo, el precio por metro cuadrado de las viviendas altas y bajas en el barrio de Villaverde no se ve influido por la altura de la vivienda. Ambas afirmaciones cuentan con un **nivel de confianza del 95%** en el método.

### Predicción del precio de una vivienda

Por último, se desea crear un modelo de regresión lineal que permita predecir el precio de una vivienda dadas sus características físicas, es decir, solo aquellas pertenecientes a la vivienda y no al anuncio.
...
Además, se puede obtener información adicional del modelo. En primer lugar, se observa que si la vivienda se encuentra en el barrio de Villaverde, el precio de la misma baja drásticamente (-352.919,2€ menos que si la vivienda se encontrara en el barrio de Salamanca). Además, como ya se había visto anteriormente en el estudio de correlaciones, cuantos más m$^2$ tenga la vivienda, mayor será su precio.

Por último, cabe destacar que tanto los m$^2$ como el barrio donde está situada la vivienda son estadísticamente significativos para el modelo; por otro lado, la planta en que se encuentra no es estadísticamente significativa. Se procede a crear un segundo modelo de regresión lineal sin esta última variable:
...
Se observa que el modelo sigue siendo estadísticamente significativo, así como los coeficientes de la localización y m$^2$ de la vivienda. El coeficiente de determinación $R^2_1 = 0.813$ frente al coeficiente de determinación del modelo completo $R^2_0 = 0.819$ indica que la planta en que se encuentra la vivienda solo captura un 0.6% de la variación de precios de las viviendas.
